...
async function pathCasFilesGenerator(selectedYear, userId, reportType) {
  ...
  if (casData) {
    if (casData) {
      ...
      newCasArray.sort((a, b) => {
        return (
          parseInt(a.casYear.slice(0, 4)) - parseInt(b.casYear.slice(0, 4))
        );
      });
      ...
      if (reportType === "CAS") {
        ...
        if (casData?.["stage5"]) {
          eligData = JSON.parse(casData["stage5"]);
          level = "stage5";
  ...
  try {
    ...
    let { eligProofs, impactProof, activityProof, directorProof } =
      fetchEligibilityProofs(
        reportType,
        level,
        eligData,
    ...
    casDataSpecifier.forEach((casSpecifier) => {
      casArray.forEach((casItem) => {
        const dataMap =
          casItem.academicData[casSpecifier.casName]?.dataMap || [];
    ...
    casDataSpecifier.forEach((casSpecifier) => {
      selectedYear.forEach((year) => {
        let filteredItems = response[casSpecifier.model].filter((item) => {
          return mainDataMap[year][
            casSpecifier.casName === "conference"
              ? "ConferenceBookAndChapter"
              : casSpecifier.casName === "publicationData"
              ? "MainBookAndChapter"
              : casSpecifier.model
          ].includes(item._id.toHexString());
        });
    ...
    let files = [
      ...(eligProofs || []),
      ...new Set(
        combineAllObjects.map((item) =>
          path.join(__dirname, `../uploads/faculty-uploads/${item.proof}`)
        )
      ),
      ...(impactProof || []),
      ...(directorProof || []),
    ...
    const fileName = `CASReport-Proofs-${new Date().getTime()}.pdf`;
